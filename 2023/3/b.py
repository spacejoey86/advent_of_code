...
for line in open("input.txt"):
    ...
    for c in line:
        ...
        if c in numbers:
            ...
            inNumber = False
        schematic[-1].append((0, 1) if (c == "*") else (0,0))
        currentCol += 1
        ...
        for x in [-1,0,1]:
            for y in [-1,0,1]:
                if inRange(nRow := row + y, nCol := col + x):
                    numAdjacent, product = schematic[nRow][nCol]
                    if not part["adjacent"] and product > 0:
                        schematic[nRow][nCol] = (numAdjacent + 1, product * int(part["val"]))
                        part["adjacent"] = True
